...
import Animated, { Easing, useAnimatedProps, useAnimatedReaction, useAnimatedScrollHandler, useAnimatedStyle, useSharedValue, withTiming } from 'react-native-reanimated';
import Header from './Header23';
import SectionTitle from './SectionTitle';
import Cell from './Cell';
import { Color, Painter ,Text as TextPainter} from 'react-native-painter';
...
const App = (props:any)=>{
    ...
    const eventProps:any = Platform.OS === 'web' ? { 
        onScroll:(e:any,ox:number,oy:number)=>{
            sharedIndex.value = oy / ItemHeight
          }
    } : {
        animatedEvent:  useAnimatedScrollHandler((event,ctx)=>{
            const {y} = event.contentOffset;
            sharedIndex.value = y / ItemHeight
        }) 
    }
    ...
    const currentTitleOffsetNextDown = useSharedValue(-100) //the more negative 
