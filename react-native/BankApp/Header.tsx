...
const Header = (props:{animatorValue:Animated.SharedValue<number>})=>{
    ...
    const [animatedHeaderStyle,setAnimatedHeaderStyle] = getAnimatedStyle({},useAnimatedStyle(()=>{
    ...
    const [animatedBalanceStyle,setAnimatedBalanceStyle] = getAnimatedStyle({},useAnimatedStyle(()=>{
   ...
   (v)=>{
       if(Platform.OS === 'web'){
            const inter = interpolate(v,input,[1,1,0,0])
            const inter2 = interpolate(v,input2,[0,0,1,1])
            runOnJS(setAnimatedHeaderStyle!)({ 
                height: inter * HeaderOptionsHeight,
                transform: [{perspective: 400},{rotateX:`${(inter2)*90}deg`}]
            })
            const interBalance = interpolate(v,input,[0,0,0,1])
           runOnJS(setAnimatedBalanceStyle!)({  transform: [{translateY: -(interBalance * ItemHeight)}]})
       }
