...
import AnimatedHelper, { getAnimatedProps } from "./reanimated";
...
const Wave = (
  ...
  const [animatedProps,setAnimatedProps] = getAnimatedProps({}, useAnimatedProps(() => {


    const offsetY = side === Side.RIGHT ? HEIGHT*0.1 : 0 //moving curve when rotating 180 deg
    const yValue = side === Side.RIGHT ? HEIGHT-y.value : y.value
    ...
    const p1 = { x: ledge.value, y:yValue - 2 * stepY };
    const p2 = vec2(p1.x + stepX, p1.y + stepY - offsetY);
    ...
    const c11 = vec2(p1.x, p1.y + C - offsetY);
  ...
  if(Platform.OS === 'web'){
    useAnimatedReaction(()=>[x.value,y.value,R.value,ledge.value] ,(v)=>{
    
      const yValue = side === Side.RIGHT ? HEIGHT-v[1] : v[1]
      const stepY = v[0] - MIN_LEDGE  ; // R = 50
      const stepX = v[2] / 2; // R/2
      // 0.5522847498 is taken from https://spencermortensen.com/articles/bezier-circle/
      ...
      const p1 = { x: v[3], y:yValue - 2 * stepY };
  ...
  return (
    ...
    <AnimatedHelper.ClipPath style={StyleSheet.absoluteFill}
 
        svgKey={`${children.props.slide.color}`}
        animatedProps={animatedProps}
        rot={ side === Side.RIGHT ? 180 : 0}
        rotO={0.5}
