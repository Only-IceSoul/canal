...
const Button = ({ position, side, activeSide }: ButtonProps) => {
  ...
  const [style,setStyle] = getAnimatedStyle({},animatedStyle)



  if(Platform.OS === 'web'){
    useAnimatedReaction(()=> [position.x.value,position.y.value,activeSide.value],(v)=>{
      runOnJS(setStyle!)({
        position: 'absolute',
        left: isLeft ? v[0] - RADIUS * 2 : width - v[0],
        top: v[1] - RADIUS,
        borderRadius: RADIUS,
        width: RADIUS * 2,
        height: RADIUS * 2,
